...
| [`TRE-FIND-004`](findings/TRE-FIND-004.md) | OPEN | `run-20260827T163820Z` + `run-20260827T174917Z` + `run-20260827T181300Z` | - | Three-level timeout/actionability qualification proved a cross-layer false-pass safety gap: when the original test-id still resolves uniquely but is non-actionable, broad timeout delegation can let TRE redirect CLICK/FILL to a different actionable locator and make the unchanged business test pass; correction ownership is framework-primary with a required TRE defense-in-depth guard. |
